...
class Row : WidgetGroup(), ChildrenAware {
    init {
    ...
    private val cellWidths = FloatSeq()
    ...
    override fun layout() {
        if (sizeInvalid) computeSize()
        val arrangeWidth = sizeMainAxis(width, prefWidth, minWidth, cells, { getPrefWidth() }, { getMinWidth() }, cellWidths)
        // End of where the helper function would be
        outPositions.setSize(cells.size)
        arrangement.arrange(arrangeWidth, cellWidths, outPositions)
        for ((i, cell) in cells.withIndex()) {
            val elem = cell.elem
            val align = cell.align

            // How much free space is left
            val areaW = cellWidths[i] - cell.padLeft - cell.padRight
            ...
            var x = outPositions[i] + cell.padLeft.toFloat()
